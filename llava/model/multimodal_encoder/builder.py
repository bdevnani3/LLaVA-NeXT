...
from .siglip_encoder import SigLipMultiModalEncoder, CLIPMultiModalEncoder, DINOVisionEncoder
...
def build_vision_tower(vision_tower_cfg, **kwargs):
    ...
    load_vision_and_text_filter = kwargs.pop("load_vision_and_text_filter", False)
    if is_absolute_path_exists or vision_tower.startswith("openai") or vision_tower.startswith("laion") or "ShareGPT4V" in vision_tower:
        if use_s2:
            return CLIPVisionTowerS2(vision_tower, args=vision_tower_cfg, **kwargs)
        else:
            return CLIPVisionTower(vision_tower, args=vision_tower_cfg, **kwargs)
    elif "siglip" in vision_tower and load_vision_and_text_filter:
        # return SigLipMultiModalEncoder(vision_tower, **kwargs)
        # return CLIPMultiModalEncoder(**kwargs)
        return DINOVisionEncoder(**kwargs)
